...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        #self.conv_7 = tf.keras.layers.Conv2D(filters=512, kernel_size=(4,2), strides=(
        ...
        self.lstm_2 = tf.keras.layers.LSTM(self.lstm_units, return_sequences=True)
    ...
    def call(self, inputs):
        ...
        lstm_input = tf.transpose(conv_layer_3, [0,2,1,3])
        s = lstm_input.shape
    ...
    def accuracy(self, logits, labels):
        ...
        for i in range(decoded.shape[0]):
            ...
            if predicted.shape[0] == 4 and np.all(predicted == labels[i]):
                results.append(1)
            else:
                results.append(0)
