...
def train(model, train_inputs, train_labels, train_lengths, train_losses):
    """
    Creates a visualization of the output of the CRNN
    :param model - the CRNN model that will be trained
    :param train_inputs - training images (has size [train_set_size, 60, 160, 1])
    :param train_labels - labels for training images (has size [train_set_size, max_sequence_length])
    :param train_lengths - actual lengths of labels (has size [train_set_size])
    :param train_losses - python list of training losses for each batch
    """
    train_inputs = tf.reshape(train_inputs, (-1, IMAGE_HEIGHT, IMAGE_WIDTH, 1))
...
def test(model, test_inputs, test_labels):
    ...
    :param test_inputs - testing images (has size [test_set_size, 60, 160, 1])
    :param test_labels - labels for testing images (has size [test_set_size, max_sequence_length])
    """
    sum = 0
    num_batches = 0
...
def visualize_results(image_inputs, logits, image_labels):
    """
    Creates a visualization of the output of the CRNN
    :param image_inputs - testing images (has size [9, 60, 160, 1])
    :param logits - output of CRNN on testing images (has size [9, num_timesteps, num_classes])
    :param image_labels - labels of testing images (has size [9, max_sequence_length])
    """
